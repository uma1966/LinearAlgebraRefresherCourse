...
class Hyperplane(object):
    ...
    DIMENSION_OR_NORMAL_VECTOR_MUST_BE_SET = 'Either dimension or normal vector must be set'

    def __init__(self, dimension=None, normal_vector=None, constant_term=None):

        if not dimension and not normal_vector:
            raise Exception(Hyperplane.DIMENSION_OR_NORMAL_VECTOR_MUST_BE_SET)
        elif not normal_vector:
            # Init normal vector with 0s
            self.dimension = dimension
            all_zeros = ['0']*self.dimension
            self.normal_vector = Vector(all_zeros)
        else:
            # Take given normal vector:
